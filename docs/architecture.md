...
| `computer` | `BrowserProvider` interface, per-tenant profiles and sessions, CDP WebSocket broker, `atlas__computer` MCP server, takeover grants, downloads relay, minute metering. | database, secrets, contracts, billing, runtime-client |
...
### 5.6 The agent's computer

Each tenant has a remote browser with a persistent profile, brokered by
ATLAS (ADR-0016, `docs/computer.md`). Playwright MCP inside the runtime
drives it over CDP through `wss://<atlas>/computer/v1/cdp`; ATLAS maps the
runtime to the tenant's profile, creates or reuses the provider session,
meters `computer_minute`, and proxies. When the agent hits a login or 2FA
wall it calls `atlas__computer.request_human_help`, ATLAS posts a link into
the Slack thread, the member logs in from the ATLAS Computer page and hands
back, and the agent continues. Logins persist in the tenant profile across
runtime restarts and upgrades. Downloads are relayed into the runtime's
...
| `ATLAS_COMPUTER_CDP_URL` | `wss://<atlas>/computer/v1/cdp`; Jarvis registers Playwright MCP against it at boot (J10). |
...
| J10 | Computer: add `@playwright/mcp` to the image; when `ATLAS_COMPUTER_CDP_URL` is set, register it as a stdio MCP server with alias `computer` and `--cdp-endpoint`; extend the system prompt with the untrusted-web-content and ask-for-human-help rules; pass conversation identity headers on MCP calls (extends J2). Later remove bundled Chrome. | Slice 6a. |
